...
from services.mailbox_service import create_tender_from_mailbox_message, import_mailbox_message_to_tender, mailbox_is_configured, sync_mailbox
...
def index():
    ...
    messages = MailboxMessage.query.order_by(MailboxMessage.received_at.desc().nullslast(), MailboxMessage.created_at.desc()).limit(50).all()
...
def sync():
    ...
    try:
        result = sync_mailbox(current_app.config["DATA_DIR"])
        flash(f"Mailbox sync complete. Created {result['created']} message(s), updated {result['updated']}.", "success")
    except Exception as exc:
        flash(f"Mailbox sync failed: {exc}", "danger")
    if tender_id:
        return redirect(url_for("mailbox.index", tender_id=tender_id))
...
def create_tender_from_message(message_id: int):
    ...
    try:
        ...
        return redirect(url_for("mailbox.view_message", message_id=mailbox_message.id))
...
def import_to_tender(message_id: int):
    ...
    try:
        ...
        return redirect(url_for("mailbox.view_message", message_id=mailbox_message.id, tender_id=tender.id))
